...
/**
 * What Kestrel costs, measured rather than claimed, over two paths.
 *
 ...
 * nobody runs.
 *
 * The other sends the shipped `kestrel-http` step at a target in this process,
...
private fun overSocketSection(measured: List<Measured>): List<String> {
    ...
    return listOf(
        ...
        "## Over a socket",
        "",
        "The shipped `kestrel-http` step — one blocking `send` per virtual thread through",
...
private fun nullStepSection(measured: List<Measured>): List<String> {
    ...
    return listOf(
        ...
        "the profile promised and the sample the recorder took is Kestrel and nothing",
