 ...
 *
 * Every other test here asserts on numbers Kestrel produced from samples it was
...
class CalibrationTest {
    ...
    @LoadTest
    fun `a target that takes forty milliseconds is reported as taking forty milliseconds`(kestrel: Kestrel) {
        val api = http.baseUrl("http://localhost:${server.address.port}")
        val hitting = io.github.matthewjones372.kestrel.scenario("calibration") { exec(api.get("/slow")) }

        val result = kestrel.run(hitting.at(20.perSecond, over = 2.seconds))
