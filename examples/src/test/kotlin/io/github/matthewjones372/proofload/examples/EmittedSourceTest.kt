...
import io.github.matthewjones372.proofload.plan.asKotlin
...
/**
 * `proofload emit` claims the source it prints compiles. A golden cannot show
 ...
 *
 * `-Dproofload.regenerate=true` rewrites it and fails, so a change to the emitter
...
class EmittedSourceTest {
    ...
    private fun checkEmitted(plan: String, into: String, source: String) {
        ...
        if (System.getProperty("proofload.regenerate").toBoolean()) {
            ...
            error("rewrote $path. Re-run without -Dproofload.regenerate and read the diff.")
        }
    ...
    private companion object {
        const val PACKAGE = "io.github.matthewjones372.proofload.examples"
        const val SOURCES = "src/main/kotlin/io/github/matthewjones372/proofload/examples"
    }
}
