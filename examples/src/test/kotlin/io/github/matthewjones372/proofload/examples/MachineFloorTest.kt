...
import io.github.matthewjones372.proofload.engine.calibrate
...
class MachineFloorTest {
    ...
    fun `the floor a test consults is measured once and is the same every time it is asked`() {
        ...
        val first = proofload.calibrate()
        val again = proofload.calibrate()
        val fromAnotherTest = Proofload().calibrate()

        again shouldBe first
        withClue("one per test method, one floor: a second Proofload must not re-measure the machine") {
