...
class InjectionProfileTest {
    ...
    fun `a constant rate departs on the interval, starting at zero`() {
        val departures = constantRate(perSecond = 4.0, over = 1.seconds).departures().toList()
    ...
    fun `a constant rate sends rate times duration users`() {
        constantRate(perSecond = 50.0, over = 60.seconds).userCount() shouldBe 3000L
    }

    @Test
    fun `a rate of zero departs nobody`() {
        constantRate(perSecond = 0.0, over = 10.seconds).departures().toList().shouldBeEmpty()
    }

    @Test
    fun `a ramp with equal ends is a constant rate`() {
        val ramp = rampRate(from = 4.0, to = 4.0, over = 1.seconds).departures().toList()

        ramp shouldBe constantRate(perSecond = 4.0, over = 1.seconds).departures().toList()
    }

    @Test
    fun `a ramp sends the area under the line`() {
        rampRate(from = 0.0, to = 100.0, over = 10.seconds).userCount() shouldBe 500L
    }

    @Test
    fun `a ramp gets faster, so its gaps shrink`() {
        val departures = rampRate(from = 1.0, to = 20.0, over = 10.seconds).departures().toList()
    ...
    fun `every departure lands inside the window it was given`() {
        val over = 10.seconds
        val departures = rampRate(from = 1.0, to = 20.0, over = over).departures().toList()
    ...
    fun `a departure is computed from its index, so a long run does not drift`() {
        val hour = constantRate(perSecond = 1000.0, over = 3600.seconds)
    ...
    fun `a negative rate is a bug in the caller, not a failure to report`() {
        shouldThrow<IllegalArgumentException> { constantRate(perSecond = -1.0, over = 1.seconds) }
        shouldThrow<IllegalArgumentException> { rampRate(from = 0.0, to = -1.0, over = 1.seconds) }
        shouldThrow<IllegalArgumentException> { constantRate(perSecond = 1.0, over = (-1).seconds) }
    }
}
