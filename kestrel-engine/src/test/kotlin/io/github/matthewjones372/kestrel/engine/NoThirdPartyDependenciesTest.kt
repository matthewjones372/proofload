...
class NoThirdPartyDependenciesTest {
    ...
    fun `the main runtime classpath is the Kotlin standard library and kestrel-core, and nothing else`() {
        val raw = System.getProperty("kestrel.engine.runtimeClasspath")
        withClue("the build must pass -Dkestrel.engine.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
        ...
        withClue("kestrel-engine must carry only kestrel-core, but found: $unexpected") {
