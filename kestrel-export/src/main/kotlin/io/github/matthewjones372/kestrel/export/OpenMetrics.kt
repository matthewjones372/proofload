...
public fun RunResult.openMetrics(run: String = startedAt.toString()): String =
    (
        latencyFamily(run) +
            distributionFamily("kestrel_behind_seconds", BEHIND_HELP, behind, mapOf("run" to run)) +
            distributionFamily("kestrel_hiccups_seconds", HICCUPS_HELP, hiccups, mapOf("run" to run)) +
            requestFamily(run) +
            failureFamily(run) +
            machineFamily() +
            listOf("# EOF")
...
private fun RunResult.latencyFamily(run: String): List<String> {
    ...
    return listOf(
        "# TYPE kestrel_latency_seconds histogram",
        "# UNIT kestrel_latency_seconds seconds",
        "# HELP kestrel_latency_seconds $LATENCY_HELP",
    ) + series.flatMap { (labels, timing) -> bucketLines("kestrel_latency_seconds", labels, timing) }
}
...
private fun RunResult.requestFamily(run: String): List<String> {
    ...
    return listOf(
        "# TYPE kestrel_requests counter",
        "# HELP kestrel_requests $REQUESTS_HELP",
    ) + steps.values.sortedBy { it.name }.map { step ->
        "kestrel_requests_total${mapOf("run" to run, "step" to step.name).asLabels()} ${step.count}"
    }
}
...
private fun RunResult.failureFamily(run: String): List<String> {
    ...
    return listOf(
        "# TYPE kestrel_failures counter",
        "# HELP kestrel_failures $FAILURES_HELP",
    ) + counted.map { (labels, count) -> "kestrel_failures_total${labels.asLabels()} $count" }
}
...
private fun RunResult.machineFamily(): List<String> = listOf(
    "# TYPE kestrel_machine info",
    "# HELP kestrel_machine $MACHINE_HELP",
    "kestrel_machine_info" +
        mapOf(
