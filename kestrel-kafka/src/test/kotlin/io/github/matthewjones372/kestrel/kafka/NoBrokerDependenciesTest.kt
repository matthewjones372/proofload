...
class NoBrokerDependenciesTest {
    ...
    private val entries: List<String>
        get() {
            val raw = System.getProperty("kestrel.kafka.runtimeClasspath")
            withClue("the build must pass -Dkestrel.kafka.runtimeClasspath; see build.gradle.kts") {
                raw.shouldNotBeNull()
            }
    ...
    fun `what is here is kestrel-core, kafka-clients and what kafka-clients itself brings`() {
        ...
        withClue("kestrel-kafka grew a dependency: $unexpected") { unexpected.shouldBeEmpty() }
    }
}
