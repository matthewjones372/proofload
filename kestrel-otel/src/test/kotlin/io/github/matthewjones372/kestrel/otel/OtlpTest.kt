...
import io.github.matthewjones372.kestrel.export.openMetrics
import io.github.matthewjones372.kestrel.timing
...
class OtlpTest {
    ...
    fun `the boundaries and the counts are this histogram's own, not re-bucketed`() {
        val point = Fixture.run.metricData(run = "the-run")
            .single { it.name == "kestrel.latency" && it.histogramData.points.any { p -> p.isOk() } }
    ...
    fun `two reasons are two points, counted apart`() {
        val failures = Fixture.run.metricData(run = "the-run").single { it.name == "kestrel.failures" }
