...
// lightly — but the rule it holds to is *per module*: kestrel-kafka carries
// kafka-clients, kestrel-otel carries the OTel SDK, kestrel-core carries
...
application {
    mainClass.set("io.github.matthewjones372.kestrel.record.MainKt")
}
...
tasks.test {
    ...
    systemProperty("kestrel.record.checkoutHar", rootProject.file("docs/examples/checkout.har").absolutePath)
    ...
    jvmArgumentProviders.add(
        CommandLineArgumentProvider {
            listOf(
                "-Dkestrel.record.runtimeClasspath=" +
