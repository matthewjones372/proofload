...
class GeneratingTest {
    ...
    private val har: Path get() = Path.of(checkNotNull(System.getProperty("kestrel.record.checkoutHar")))

    private val checkedIn: Path =
        Path.of("src/test/kotlin/io/github/matthewjones372/kestrel/record/generated/Checkout.kt")

    private fun generated(): String =
        draft(readHar(Files.readString(har)))
            .asKotlin(
                packageName = "io.github.matthewjones372.kestrel.record.generated",
                scenario = "checkout",
                from = "checkout.har",
            )
    ...
    fun `the cookbook's own checkout regenerates to the file checked in beside it`() {
        ...
        if (System.getProperty("kestrel.regenerate") == "true") {
            Files.writeString(checkedIn, regenerated)
        }

        withClue("run with -Dkestrel.regenerate=true and read the diff before keeping it") {
