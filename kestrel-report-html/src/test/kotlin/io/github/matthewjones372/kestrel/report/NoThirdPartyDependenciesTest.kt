...
class NoThirdPartyDependenciesTest {
    ...
    fun `the main runtime classpath is the Kotlin standard library and kestrel-core, and nothing else`() {
        val raw = System.getProperty("kestrel.report.html.runtimeClasspath")
        withClue("the build must pass -Dkestrel.report.html.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
        ...
        withClue("kestrel-report-html must carry no library of its own, but found: $unexpected") {
