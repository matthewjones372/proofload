...
class NoThirdPartyDependenciesTest {
    ...
    private fun classpath(): List<String> {
        val raw = System.getProperty("proofload.arbs.runtimeClasspath")
        withClue("the build must pass -Dproofload.arbs.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
    ...
    fun `the main runtime classpath is the Kotlin standard library and proofload-core, and nothing else`() {
        ...
        withClue("proofload-arbs may depend on proofload-core and the JDK only, but found: $unexpected") {
    ...
    fun `proofload-core is on the runtime classpath`() {
        ...
        withClue("proofload-arbs fills core's sessions, so core must be exported: ${classpath()}") {
