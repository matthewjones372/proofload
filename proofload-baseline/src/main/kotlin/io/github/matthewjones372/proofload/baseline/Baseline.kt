...
import io.github.matthewjones372.proofload.ArrivalSeries
...
internal fun parseBaseline(text: String): RunResult {
    ...
    require(header.getOrNull(0) == MARKER) { "not a Proofload baseline: ${lines.firstOrNull()}" }
    // Named rather than merely refused. A version this build cannot read is a
    // Proofload somewhere else, and which one is the only useful thing to say.
    require(header.getOrNull(1) in READABLE) {
        "this baseline is version ${header.getOrNull(1)}; this Proofload reads " +
...
private const val MARKER = "proofload-baseline"
