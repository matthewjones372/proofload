...
class BaselineTest {
    ...
    private fun RunResult.throughAFile(dir: Path): RunResult {
        writeBaseline(dir.resolve("b.proofload"))
        return readBaseline(dir.resolve("b.proofload"))
    }

    @Test
    fun `a run written and read back is the same run to compare against`(@TempDir dir: Path) {
        val run = runOf()
        run.writeBaseline(dir.resolve("baseline.proofload"))

        val read = readBaseline(dir.resolve("baseline.proofload"))
    ...
    fun `the buckets travel, because an interval cannot be rebuilt from five numbers`(@TempDir dir: Path) {
        val run = runOf()
        run.writeBaseline(dir.resolve("b.proofload"))

        val read = readBaseline(dir.resolve("b.proofload"))
    ...
    fun `a slower run compares as worse against what was kept`(@TempDir dir: Path) {
        runOf(range = 80L..120L).writeBaseline(dir.resolve("b.proofload"))

        val comparison = runOf(range = 800L..1_200L).against(readBaseline(dir.resolve("b.proofload")))
    ...
    fun `a step named with a tab cannot split a line`(@TempDir dir: Path) {
        ...
        awkward.writeBaseline(dir.resolve("b.proofload"))

        readBaseline(dir.resolve("b.proofload")).steps.keys shouldBe setOf("pay\tnow")
    }
    ...
    fun `a baseline from a future version is refused, and the refusal names the version`() {
        val why = shouldThrow<IllegalArgumentException> {
            parseBaseline("proofload-baseline\t99\nrun\t2026-08-26T09:00:00Z\n")
        }.message.orEmpty()
    ...
    fun `the probe travels, so a runner half as fast is named before a step is`(@TempDir dir: Path) {
        val plan = planOf(constantRate(100.perSecond, over = 2.seconds))
        runOf(plan = plan, probe = Probe(50.microseconds)).writeBaseline(dir.resolve("b.proofload"))

        val compared = runOf(plan = plan, probe = Probe(100.microseconds))
            .against(readBaseline(dir.resolve("b.proofload")))
    ...
    fun `a baseline written by the version before this one reads, and claims no probe`() {
        val older = runOf(plan = planOf(constantRate(100.perSecond, over = 2.seconds)))
            .asBaseline()
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t3")
    ...
    fun `a warmed run is refused against a baseline that warmed nothing`(@TempDir dir: Path) {
        ...
        cold.writeBaseline(dir.resolve("b.proofload"))

        val warmed = runOf(
            plan = planOf(constantRate(100.perSecond, over = 2.seconds)).copy(warmUp = WarmUp(5.seconds)),
        )
        val why = warmed.against(readBaseline(dir.resolve("b.proofload")))
    ...
    @Test
    fun `a shard carries the hold its own clock computed, which is the only place a wrong clock shows`(
        @TempDir dir: Path,
    ) {
        val shard = Shard(
            index = 2,
            of = 4,
            startingAt = Instant.parse("2026-08-26T09:00:00Z"),
            heldFor = 29_500.milliseconds,
        )
    ...
    @Test
    fun `a version 7 baseline reads, and its shard claims no hold`() {
        // What a version 7 writer produced: this file's shard line without the
        // field it did not have.
        val shard = Shard(index = 2, of = 4, startingAt = Instant.parse("2026-08-26T09:00:00Z"), heldFor = 1.seconds)
        val older = runOf().copy(shard = shard).asBaseline()
            .lineSequence()
            .map { if (it.startsWith("shard\t")) it.split("\t").take(4).joinToString("\t") else it }
            .joinToString(separator = "\n")
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t7")

        val read = parseBaseline(older)

        withClue("a merge of files that old cannot see a clock that disagreed, and must not invent one") {
            read.shard shouldBe shard.copy(heldFor = null)
        }
    }
    ...
    fun `a version 5 baseline reads, and claims no lateness, stalls or shard`() {
        ...
        val older = runOf(plan = planOf(constantRate(100.perSecond, over = 2.seconds)))
            ...
            .joinToString(separator = "\n")
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t5")
    ...
    fun `a version 8 baseline reads, and claims nothing about what it drew from`() {
        val older = runOf(plan = drawingPlan(listOf(Shape("zipf(keys=1000000, skew=1.1)", seed = 4L))))
            ...
            .joinToString(separator = "\n")
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t8")
    ...
    fun `a version 4 baseline reads, and claims no warm-up`() {
        val older = runOf(plan = planOf(constantRate(100.perSecond, over = 2.seconds)))
            .asBaseline()
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t4")
    ...
    fun `a closed run and an open one are refused rather than compared`(@TempDir dir: Path) {
        runOf(plan = planOf(constantRate(100.perSecond, over = 2.seconds))).writeBaseline(dir.resolve("b.proofload"))
    ...
    fun `a version 6 baseline reads, since nothing could have written a closed population into one`() {
        val older = runOf(plan = planOf(constantRate(100.perSecond, over = 2.seconds)))
            .asBaseline()
            .replaceFirst("proofload-baseline\t9", "proofload-baseline\t6")
    ...
    fun `a run replayed from another capture is refused rather than compared`(@TempDir dir: Path) {
        ...
        runOf(plan = planOf(friday.replaying())).writeBaseline(dir.resolve("b.proofload"))
    ...
    fun `the plan travels, so the next run can be refused rather than compared to unlike work`(@TempDir dir: Path) {
        ...
        smoke.writeBaseline(dir.resolve("b.proofload"))

        val soak = runOf(plan = planOf(constantRate(500.perSecond, over = 2.seconds)))
        val why = soak.against(readBaseline(dir.resolve("b.proofload")))
            .shouldBeInstanceOf<Comparison.NotComparable>()
            .why
    ...
    fun `the machine travels, so a comparison can say the runner changed under it`(@TempDir dir: Path) {
        ...
        baseline.writeBaseline(dir.resolve("b.proofload"))
