 ...
 * `proofload-record` does not either. No Pekko, which is `pelican-client-pekko`'s
...
class NoThirdPartyDependenciesTest {
    ...
    @Test
    fun `the main runtime classpath is proofload, pelican-core and the one parser`() {
        val raw = System.getProperty("proofload.contract.runtimeClasspath")
        withClue("the build must pass -Dproofload.contract.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
        ...
        withClue("proofload-contract carries no server stack and no actor system, but found: $unexpected") {
