...
private class Lines(private val every: Duration) : Progress {
    ...
    override fun starting(plan: Plan) {
        ...
        println(
            "proofload: ${plan.scenario} — ${plan.plannedUsers.grouped()} users over ${plan.plannedWindow}, " +
    ...
    override fun searching(search: Search) {
        println(
            "proofload: capacity — at most ${search.rungs.size} rungs of ${search.holding} and the bisection " +
                "after them, so at most ${search.worstCase}",
        )
    }

    override fun waited(queued: Duration) {
        println("proofload: waited $queued for the machine")
    }

    override fun aligning(shard: Shard, until: Duration) {
        println("proofload: injector ${shard.index} of ${shard.of} — holding $until for ${shard.startingAt}")
    }
    ...
    override fun climbed(rung: Rung, number: Int, atMost: Duration) {
        ...
        println("proofload: rung $number — ${rung.rate.forReading()}/s $became$left")
    }
}

// The same `proofload:` prefix the report writer uses, so a run reads as one
