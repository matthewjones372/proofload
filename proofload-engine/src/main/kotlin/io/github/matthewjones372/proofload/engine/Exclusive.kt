...
internal fun <T> exclusively(during: Exclusivity, progress: Progress = Progress.silent, work: () -> T): T {
    ...
    // `-Dproofload.exclusive=false` is for a caller that means to run several at
...
private fun exclusivityWanted(): Boolean = System.getProperty("proofload.exclusive")?.toBooleanStrictOrNull() != false
