...
fun Scenario.trace(feeder: Feeder = Feeder.empty) {
    ...
    runOneUser(
        ...
        narrating = { _, notes -> notes.forEach { println("proofload:   ${" ".repeat(width)}  $it") } },
    )
}
...
private fun printedTo(width: Int): StepSink = StepSink { step, failure, _, _, _, _, _, _, _ ->
    println("proofload:   ${step.padEnd(width)}  ${failure.outcome()}")
}
