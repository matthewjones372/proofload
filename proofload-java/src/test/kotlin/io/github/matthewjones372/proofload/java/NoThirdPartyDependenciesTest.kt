...
class NoThirdPartyDependenciesTest {
    ...
    private val exported = listOf("proofload-core", "proofload-engine", "proofload-http")

    private val allowed = exported + listOf("kotlin-stdlib", "annotations-")

    private fun classpath(): List<String> {
        val raw = System.getProperty("proofload.java.runtimeClasspath")
        withClue("the build must pass -Dproofload.java.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
    ...
    fun `the main runtime classpath is the modules it delegates to, and nothing else`() {
        ...
        withClue("proofload-java may depend on $exported and the JDK only, but found: $unexpected") {
