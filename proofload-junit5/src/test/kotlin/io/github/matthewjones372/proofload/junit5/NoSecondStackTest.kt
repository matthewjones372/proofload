...
class NoSecondStackTest {
    ...
    @Test
    fun `the main runtime classpath is proofload, JUnit, and nothing else`() {
        val raw = System.getProperty("proofload.junit5.runtimeClasspath")
        withClue("the build must pass -Dproofload.junit5.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
        ...
        withClue("proofload-junit5 grew a dependency: $unexpected") { unexpected.shouldBeEmpty() }
        withClue("the engine is what runs a simulation, so it must be here") {
            entries.map { it.substringBefore("-0") } shouldContain "proofload-engine"
        }
    }
}
