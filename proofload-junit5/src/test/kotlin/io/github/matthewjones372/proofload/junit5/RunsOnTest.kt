...
class RunsOnTest : RunsOn {
    ...
    @LoadTest
    fun `a load test is sent by the engine its class names`(proofload: Proofload) {
        val simulation = browsing.at(4.perSecond, over = 1.seconds)

        val result = proofload.run(simulation)
    ...
    @LoadTest
    fun `every rung of a search under a load test is sent by that same engine`(proofload: Proofload) {
    ...
    @LoadTest
    fun `a search on a named engine prints nothing either`(proofload: Proofload) {
        val search = browsing.sustainable(
            upTo = 10.perSecond,
            holding = 20.milliseconds,
            expecting = listOf(failureRate under 1.percent),
