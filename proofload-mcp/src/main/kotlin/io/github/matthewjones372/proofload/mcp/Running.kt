...
internal class Registry(
    private val proofload: () -> Proofload = { Proofload(progress = Progress.silent) },
    ...
    fun start(plan: Declaration, allowance: Allowance): String {
        ...
        when (val asked = simulation.preview(allowance)) {
            ...
            is Preview.Allowed -> {
                ...
                Thread.ofVirtual().name("proofload-mcp-$id").start {
                    byId[id] = try {
                        when (val ran = proofload().runWithin(allowance, simulation)) {
    ...
    private fun keep(id: String, finished: Progressing.Finished) {
        try {
            ...
            System.err.println("proofload: could not keep $id: ${unwritable.message}")
        }
    }
    ...
    private fun read(id: String): Progressing.Finished? {
        ...
        return try {
            ...
            System.err.println("proofload: could not read $id: ${unreadable.message}")
            null
        }
    }
 ...
 */
private val KEPT_RUNS: Path = Path.of("build", "reports", "proofload", "runs")

/** Enough of a UUID that nothing here has to think about collisions. */
private const val ID_LENGTH = 8

private const val KEPT = ".proofload"
