...
import io.github.matthewjones372.proofload.http.DeclaredStatus
    ...
    sending(arguments, allowance) { plan ->
        ...
        content(
            capturing {
                ...
                Proofload(progress = Progress.silent).trace(arm.scenario, arm.feeder)
            },
        )
    }
...
private fun onceThrough(plan: Declaration): RunResult {
    val arm = plan.asSimulation(kafkaLowerings).arms.single()
    return Proofload(progress = Progress.silent)
