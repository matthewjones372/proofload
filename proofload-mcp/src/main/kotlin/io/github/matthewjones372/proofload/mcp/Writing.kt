...
private fun RunResult.asBenchmark(plan: Declaration, why: String?): String = buildString {
    ...
    appendLine()
    appendLine("${plan.steps.joinToString { it.described() }} against `${plan.against()}`,")
    ...
    appendLine("```")
    appendLine()
    appendLine("Run it again with `proofload run`, or hand the block above to the `run` tool.")
    appendLine()
...
/** What one step sends, in the words its own protocol uses for it. */
...
private fun Declaration.against(): String = baseUrl ?: brokers ?: "nowhere named"
