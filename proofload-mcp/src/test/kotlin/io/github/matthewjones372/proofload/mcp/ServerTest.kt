...
class ServerTest {
    ...
    fun `it says what it is`() {
        exchange("""{"jsonrpc":"2.0","id":1,"method":"initialize"}""") shouldContain """"name":"proofload""""
    }
    ...
    fun `every plan in the schema is a plan the reader accepts`() {
        val examples = PLAN_SCHEMA.substringAfter("Four worked plans.")
            .split("---")
            .map { it.trim() }
            .filter { it.startsWith("proofload:") }
    ...
    private companion object {
        ...
        val KNOWN = setOf(
            "proofload", "baseUrl", "brokers", "scenario", "steps", "load", "goals",
