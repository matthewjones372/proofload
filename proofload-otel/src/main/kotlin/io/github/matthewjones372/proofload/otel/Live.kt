...
import io.github.matthewjones372.proofload.throttled
...
internal class Live(private val run: String, private val collector: Collector) : Progress {
    ...
    override fun tick(elapsed: Duration, snapshot: Snapshot) {
        ...
        if (!collector.send(metrics) && warned.compareAndSet(false, true)) {
            System.err.println("proofload: ${collector.named} $WARNING")
        }
        if (snapshot.ended) {
            ended.set(true)
            collector.close()
 ...
 *
 * `proofload.behind` is the finished run's distribution over every departure, and
...
private val SCOPE: InstrumentationScopeInfo = InstrumentationScopeInfo.create("proofload")

private val RUN: AttributeKey<String> = AttributeKey.stringKey("run")

private const val DEPARTED = "proofload.departed"
private const val IN_FLIGHT = "proofload.in_flight"
private const val REQUESTS = "proofload.requests"
private const val FAILURES = "proofload.failures"
private const val BEHIND_LAST = "proofload.behind.last"
