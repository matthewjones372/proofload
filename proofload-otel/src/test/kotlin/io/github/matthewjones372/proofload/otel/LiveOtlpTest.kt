...
class LiveOtlpTest {
    ...
    fun `the counters are what happened since the last tick, not the run so far counted again`() {
        ...
        captured.pushes.map { it.longValue("proofload.requests") } shouldContainExactly listOf(100L, 160L)
        captured.pushes.map { it.longValue("proofload.failures") } shouldContainExactly listOf(2L, 3L)
    }
    ...
    fun `a gauge is what the scheduler had at that instant`() {
        ...
        push.longValue("proofload.departed") shouldBe 500L
        push.longValue("proofload.in_flight") shouldBe 12L
        push.single { it.name == "proofload.behind.last" }.doubleGaugeData.points.single().value shouldBe 0.04
    }
    ...
    private fun onStandardError(block: () -> Unit): List<String> {
        ...
        // Proofload's own lines only: the SDK logs its own severe record for the
        // same refusal, which is the "the exporter logged why" this warning
        // points at rather than a second warning of ours.
        return captured.toString(Charsets.UTF_8).lines().filter { it.startsWith("proofload:") }
    }
