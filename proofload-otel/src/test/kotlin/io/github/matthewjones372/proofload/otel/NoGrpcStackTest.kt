 ...
 * `proofload-pelican` carries `pelican-core` — so the claim worth keeping is not
...
class NoGrpcStackTest {
    ...
    private val entries: List<String>
        get() {
            val raw = System.getProperty("proofload.otel.runtimeClasspath")
            withClue("the build must pass -Dproofload.otel.runtimeClasspath; see build.gradle.kts") {
                raw.shouldNotBeNull()
            }
    ...
    @Test
    fun `what is here is the SDK, its exporter over the JDK client, and proofload-core`() {
        withClue(entries.toString()) {
            entries.any { it.startsWith("proofload-core") } shouldBe true
    ...
    @Test
    fun `nothing outside OpenTelemetry, Kotlin and proofload came with them`() {
        val allowed = listOf("kotlin-stdlib", "annotations-", "proofload-", "opentelemetry-")

        val unexpected = entries.filterNot { entry -> allowed.any { entry.startsWith(it) } }

        withClue("proofload-otel grew a dependency outside the SDK it exists to carry: $unexpected") {
