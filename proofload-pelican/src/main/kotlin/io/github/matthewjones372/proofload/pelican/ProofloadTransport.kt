 ...
 */
fun proofloadTransport(
    recorder: RunRecorder? = null,
    templates: List<String> = emptyList(),
    timeout: Duration = REQUEST_TIMEOUT,
): ClientTransport = ProofloadTransport(recorder, templates.map(::Template), timeout)

private class ProofloadTransport(
...
/**
 * The same fact `proofload-http`'s `HttpStatus` is, and a separate type because
