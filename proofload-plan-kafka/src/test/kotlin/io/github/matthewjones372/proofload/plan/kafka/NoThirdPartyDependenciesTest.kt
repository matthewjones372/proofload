 ...
 * nothing but requests takes `proofload-plan` and gets none of this.
 */
class NoThirdPartyDependenciesTest {

    private val entries: List<String>
        get() {
            val raw = System.getProperty("proofload.plan.kafka.runtimeClasspath")
            withClue("the build must pass -Dproofload.plan.kafka.runtimeClasspath; see build.gradle.kts") {
                raw.shouldNotBeNull()
            }
    ...
    fun `what is here is the plan, Kafka, and what each of them already brought`() {
        val allowed = listOf(
            ...
            "proofload-core", "proofload-http", "proofload-plan", "proofload-arbs", "proofload-kafka",
        ...
        withClue("proofload-plan-kafka grew a dependency: $unexpected") { unexpected.shouldBeEmpty() }
    }

    @Test
    fun `the Kafka client is here rather than under the plan, which is the reason this module exists`() {
        withClue("if kafka-clients were reachable from proofload-plan, this module would be pointless") {
