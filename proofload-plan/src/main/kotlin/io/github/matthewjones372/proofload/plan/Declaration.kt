...
import io.github.matthewjones372.proofload.arbs.Arb
...
import io.github.matthewjones372.proofload.http.http
import io.github.matthewjones372.proofload.p50
import io.github.matthewjones372.proofload.p95
...
/**
 * A plan somebody wrote down, before it is anything Proofload can run.
 *
 ...
 *
 * `proofload-plan` carries the JDK's HTTP client and lowers a request itself.
...
/**
 * A generator a plan names, in the shapes `proofload-arbs` offers.
 *
    ...
    return lowerings.firstNotNullOfOrNull { it.lower(step, this) }
        ?: throw IllegalArgumentException(
            "nothing here lowers the step `${step.name}`, which $what: " +
                "add `proofload-plan-kafka` and pass its lowering to asSimulation",
        )
}
