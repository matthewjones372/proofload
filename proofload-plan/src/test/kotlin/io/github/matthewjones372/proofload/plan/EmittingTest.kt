...
class EmittingTest {
    ...
    private val emitted = readPlan(PLAN).asKotlin(
        packageName = "io.github.matthewjones372.proofload.examples",
        from = "checkout.yaml",
    )
    ...
    fun `the header says where it came from and does not claim to be a recording`() {
        withClue(emitted) {
            emitted shouldContain "Written from checkout.yaml by `proofload emit`"
    ...
    fun `a produce step emits the topic once and a step that sends to it`() {
        ...
        ).asKotlin(packageName = "io.github.matthewjones372.proofload.examples", from = "orders.yaml")
        ...
        withClue(emitted) {
            ...
            withClue("a plan of nothing but topics names no HTTP client and imports none") {
                ...
                emitted shouldNotContain "import io.github.matthewjones372.proofload.http.http"
            }
        }
    }
