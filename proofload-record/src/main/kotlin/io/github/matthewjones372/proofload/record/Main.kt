...
fun main(args: Array<String>) {
    ...
    println("proofload: ${drafted.steps.size} steps from ${recorded.size} recorded requests, at ${asked.file}")
    if (drafted.captures.isNotEmpty()) {
        println("proofload: captured ${drafted.captures.joinToString { it.name }}")
    }
    val credentials = drafted.steps.sumOf { it.dropped.size }
    if (credentials > 0) println("proofload: $credentials credential header(s) dropped, each left as a TODO")
}
...
private val usage = """
    proofload-record — a HAR read into a Kotlin scenario you edit and commit.
