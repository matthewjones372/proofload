 ...
 * proofload-http does not get a JSON parser because a recorder wanted one.
 */
class NoThirdPartyDependenciesTest {

    private val allowed = listOf("kotlin-stdlib", "annotations-", "proofload-core", "kotlinx-serialization")

    private fun classpath(): List<String> {
        val raw = System.getProperty("proofload.record.runtimeClasspath")
        withClue("the build must pass -Dproofload.record.runtimeClasspath; see build.gradle.kts") {
            raw.shouldNotBeNull()
        }
    ...
    @Test
    fun `what is here is proofload-core, the Kotlin standard library and the JSON parser it exists to use`() {
        val unexpected = classpath().filterNot { entry -> allowed.any { entry.startsWith(it) } }

        withClue("proofload-record grew a dependency beyond the parser it carries: $unexpected") {
    ...
    fun `proofload-core is on the runtime classpath`() {
        classpath().filter { it.startsWith("proofload-core") }.shouldNotBeEmpty()
    }
    ...
    fun `no module depends on this one, so nobody else pays for the parser`() {
        ...
        val declaring = Files.list(root).use { entries ->
            entries.filter { it.fileName.toString().startsWith("proofload-") }
                .filter { it.fileName.toString() != "proofload-record" }
     ...
     * "build-time tooling, like `proofload-record`" — is a reader being told
     * where the precedent is, not a consumer paying for a parser.
     */
    private fun String.declaresRecord(): Boolean =
        contains("proofload-record") && substringBefore("//").contains("project(")
}
