...
import io.github.matthewjones372.proofload.Stage
import io.github.matthewjones372.proofload.StepStats
import io.github.matthewjones372.proofload.TIGHT
...
import io.github.matthewjones372.proofload.heldScheduleFor
import io.github.matthewjones372.proofload.inFlight
...
import io.github.matthewjones372.proofload.stages
import io.github.matthewjones372.proofload.unanswered
import io.github.matthewjones372.proofload.unmatched
...
internal fun announce(written: Path): Path {
    println("proofload: report at ${written.toAbsolutePath().normalize().toUri()}")
    return written
}
...
private fun RunResult.cutShortLines(): List<String> {
    ...
    return listOf(
        """  <p class="behind" id="proofload-cut-short" role="status">""",
...
private fun RunResult.lostLines(): List<String> =
    if (unanswered.isEmpty()) emptyList()
        ...
        """  <p class="behind" id="proofload-lost" role="status">""",
...
private fun Floor?.resolutionLines(): List<String> =
    if (this == null || !supportsAClaim) emptyList()
        ...
        """  <p class="note" id="proofload-floor">Calibrated on this machine: differences under """ +
...
private fun RunResult.closedLines(): List<String> {
    ...
    return listOf(
        """  <p class="behind" id="proofload-closed" role="status">""",
...
private fun RunResult.behindLines(): List<String> =
    if (!fellBehind()) emptyList()
        ...
        """  <p class="behind" id="proofload-behind" role="status">""",
...
private fun RunResult.headLines(): List<String> =
    listOf(
        ...
        "<title>Proofload run — ${started()}</title>",
        "<style>",
 ...
 * samples rather than run has no plan to name, and "Proofload run" is still true
...
private fun RunResult.concurrencyLines(): List<String> {
    ...
    return if (law.agrees) {
        listOf(
            """  <p class="note" id="proofload-concurrency">Little's law holds on this run: $sides, """ +
                "a ratio of ${law.ratio.round()} across ${law.samples} " +
                "${if (law.samples == 1) "sample" else "samples"}.$queue</p>",
        )
    } else {
        listOf(
            """  <p class="behind" id="proofload-concurrency" role="status">""",
...
private fun RunResult.roomLines(): List<String> {
    ...
    return listOf(
        """  <p class="behind" id="proofload-room" role="status">""",
...
private fun RunResult.attemptLines(): List<String> {
    ...
    return listOf(
        """  <p class="note" id="proofload-attempts">Some steps went to the target more than once per """ +
...
private fun RunResult.streamLines(): List<String> {
    ...
    return listOf(
        """  <p class="note" id="proofload-streams">Some steps reported more than one answer per """ +
...
private fun RunResult.hiccupLines(): List<String> =
    if (hiccups.count == 0L) emptyList()
        ...
        """  <p class="note" id="proofload-hiccups">The injector's own JVM stalled for """ +
...
private fun RunResult.dataLines(): List<String> =
    listOf("""<script type="application/json" id="proofload-run">""") +
