...
class OnlyTheScalaLibraryTest:
  ...
  private val exported = List("proofload-core", "proofload-engine", "proofload-http", "proofload-java")
  ...
  private def classpath(): List[String] =
    val raw = System.getProperty("proofload.scala.runtimeClasspath")
    assertNotNull(raw, "the build must pass -Dproofload.scala.runtimeClasspath; see build.gradle.kts")
  ...
  def `the main runtime classpath is the modules it delegates to, the Scala library, and nothing else`(): Unit =
    ...
    assertTrue(unexpected.isEmpty, s"proofload-scala may depend on $allowed only, but found: $unexpected")
