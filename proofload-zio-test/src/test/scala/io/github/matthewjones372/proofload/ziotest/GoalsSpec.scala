...
object GoalsSpec extends ZIOSpecDefault:
  ...
  private def ran(port: Int, goals: io.github.matthewjones372.proofload.Goal*): ZIO[Any, Throwable, RunResult] =
    val api = http.baseUrl(s"http://localhost:$port")
    val browsing = scenario("browsing")(exec(browse, api.get("/products").expecting(200)))
    proofload.run(Simulations.at(browsing, 20.perSecond, JavaDuration.ofMillis(300), goals*))
