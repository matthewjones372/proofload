...
import io.github.matthewjones372.proofload.engine.VirtualThreads
  ...
  def spec = suite("a load test that is a zio-test test")(
    test("runs a scenario against a target and reads what it measured"):
      ZIO.scoped:
        for
          ...
          result <- proofload.run(browsing.at(20.perSecond, over = 500.millis))
        yield assertTrue(
    ...
    test("sends the run on the blocking executor, not the pool the fiber is on"):
      ...
      for
        ...
        _ <- proofload.run(idle.at(1.perSecond, over = 50.millis), on = recording)
      yield assertTrue(
