...
- `Floor` gains the **absolute** spread beside the relative one. It is the same
  measurement expressed the way it actually transfers.
...
- [ ] **`spec-0048-consulted`** — 0038's comparison and the report reading the
      new shape.
      Done when: a comparison on a loaded machine returns a verdict rather than
      refusing everything, and a difference genuinely inside the machine's own
      movement still reports `CannotTell` naming it.
...
4. **Does this invalidate anything 0042 shipped?** No — 0042 already requires
    the absolute gate. Recommend it be re-read against this once it lands, since
    it may be able to drop its own local `separates` helper.
