# 0095 — Proofload from Scala

## Problem

Scala 3 reads Kotlin bytecode, so a Scala caller is in the same position
[0094](0094-proofload-from-java.md) describes and one step worse. The 245
...
.FiniteDuration` is what a Scala codebase writes `1.minute` in, and Proofload's
...
- **No effect-system integration — no ZIO, no cats-effect, no Pekko.** This is
  ...
  itself; what does not happen is Proofload handing back a `ZIO` or an `IO`.
...
`proofload-scala`, a thin layer over `proofload-java`'s facade:
...
val result = Proofload().run(checkout.at(50.perSecond, over = 1.minute))
...
Sitting on `proofload-java` rather than on core is the choice worth arguing. The
...
- [x] **`spec-0095-module`** — `proofload-scala`, Scala 3, its dependency test,
...
- [x] **`spec-0095-gate`** — a compiled Scala sample module, and
      `proofload-scala` added to `smoke`.
...
- **A Scala module has no `.api` dump worth checking.** BCV records a JVM ABI
  ...
  caller can type, moving on edits no caller can see. `proofload-scala` is
...
- **Does the build cross-publish `_3` artifacts?** Recommend the plain
  `proofload-scala` coordinate with no Scala suffix while there is one supported
